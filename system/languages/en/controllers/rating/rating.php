<?php

define('LANG_RATING_CONTROLLER', 'Rating');
define('LANG_RATING_DISABLED', 'You cannot vote');
define('LANG_RATING_VOTED', 'You have already voted');
define('LANG_RATING_VOTE_UP', 'Vote Up');
define('LANG_RATING_VOTE_DOWN', 'Vote Down');
define('LANG_RATING_IS_HIDDEN', 'Hide overall rating unless a user votes');
define('LANG_RATING_SHOW_INFO', 'Show voters and their votes');
define('LANG_RATING_NO_VOTES', 'No votes yet');
define('LANG_RATING_ALLOW_GUEST_VOTE', 'Allow guests vote');
define('LANG_RATING_ALLOW_CHANGE_VOTE', 'Allow to change rating');
define('LANG_RATING_ALLOW_CHANGE_VOTE_HINT', 'If the rating template suggests this');
define('LANG_RATING_DISABLE_NEGATIVE_VOTES', 'Disable negative votes');
define('LANG_RATING_ALLOW_CHANGE_VOTE_SESSION', 'Allow changing vote only within the current user session');
define('LANG_RATING_ALLOW_CHANGE_VOTE_SESSION_HINT', 'As soon as the user logs out or the authorization session automatically ends due to user inactivity');
define('LANG_RATING_TEMPLATE', 'Rating block template');
define('LANG_RATING_UP', 'positively');
define('LANG_RATING_DOWN', 'negatively');
define('LANG_RATING_PM', '%s %s rated your item <a href="%s">%s</a>');
define('LANG_RATING_NOTIFY_VOTE',  'Notify on new item vote');
define('LANG_RATING_ITEM_LABEL',  'Inscription before the item rating block');
define('LANG_RATING_LIST_LABEL',  'Inscription before the list item rating block');
define('LANG_RATING_IS_AVERAGE',  'Rating considered as the average of all evaluations');
define('LANG_RATING_IS_AVERAGE_HINT',  'Use this option if the rating logic in your template is different from the standard');
