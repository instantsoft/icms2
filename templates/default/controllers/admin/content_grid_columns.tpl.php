        ...
        <fieldset>
            ...
            <table cellpadding="2" cellspacing="0" border="0" width="100%">
                ...
                <?php foreach($fields as $name => $field){ ?>
                    <tr>
                        <td><?php echo html_checkbox('columns['.$type.']['.$name.'][enabled]', !empty($config[$type][$name]['enabled'])); ?></td>
                        <td>
                            <label><?php html($field['title']); ?></label>
                            <?php echo html_input('hidden', "columns[{$type}][{$name}][field]", $name); ?>
                        </td>
                        <td>
                            <?php if(!empty($field['filters'])){
...
<?php } ?>
        <div style="text-align:right"><?php echo html_submit(LANG_APPLY); ?>
            <span style="margin-left:15px"><?php echo html_button(LANG_CP_GRID_COLYMNS_RESET, 'reset', 'return contentGridColumnsResetSettings()'); ?></span></div>
