<?php if($options) {
    ...
    <?php foreach($options as $field) {
        ...
        <div class="form-group <?php echo implode(' ', $classes); ?>" id="f_<?php echo $field->id; ?>" <?php if ($styles) { ?>style="<?php echo implode(';', $styles); ?>"<?php } ?>>
                <?php echo $field->getInput($value); ?>
                <?php if(!empty($field->hint)) { ?><div class="hint form-text text-muted small mt-1"><?php echo $field->hint; ?></div><?php } ?>
        </div>
    <?php } ?>
<?php } ?>
