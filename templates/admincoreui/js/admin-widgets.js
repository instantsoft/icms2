...
function widgetDelete(link) {
    ...
    let id = parseInt(link) > 0 ? link : $(link).parents('li:first').attr('data-bp_id');

    if (id > 0 && !confirm(LANG_CP_WIDGET_DELETE_CONFIRM)) {
        return false;
    }

    let widget_dom = $("#cp-widgets-layout li[data-bp_id=" + id + ']');
